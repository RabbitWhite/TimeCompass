    ...
    <div>
      ...
      <div className="timeline-controls">
        {TIME_WINDOWS.map(tw => (
          <button
            key={tw.days}
            className={`timeline-chip ${windowDays === tw.days ? 'active' : ''}`}
      ...
      {state.settings.googleCalendarConnected && (
        <div className="sync-banner">
          <svg viewBox="0 0 24 24" width="20" height="20" fill="var(--success)">
      ...
      {eventsByDay.map(({ date, events }) => {
        ...
        return (
          <div className="day-group" key={date.toISOString()}>
            <div className="day-label" style={isToday ? { color: 'var(--primary)' } : {}}>
              {isToday ? 'Today' : formatDate(date.toISOString())}
            </div>
            {events.length === 0 && (
              <div className="text-secondary text-sm" style={{ padding: '8px 4px' }}>
                No events scheduled
              </div>
            )}
            {events.map(event => {
              ...
              return (
                <div className="event-card" key={event.id}>
                  ...
                  <span
                    className="event-dot"
                    style={{ background: linkedArea?.color || (event.source === 'google' ? '#4285f4' : 'var(--text-muted)') }}
                  />
                  ...
                  {event.source === 'manual' && (
                    <button
                      className="btn btn-ghost btn-sm"
                      onClick={() => dispatch({ type: 'DELETE_CALENDAR_EVENT', payload: event.id })}
                    >
                      &times;
                    </button>
                  )}
      ...
      {state.calendarEvents.length === 0 && (
        <div className="empty-state">
          ...
          <p>No upcoming events.<br />Add events manually or sync with Google Calendar.</p>
        </div>
      )}
      ...
      {showForm && (
        <Modal title="Add Event" onClose={() => setShowForm(false)}>
          <div className="form-group">
            <label className="form-label">Title</label>
            <input className="form-input" value={eTitle} onChange={e => setETitle(e.target.value)} placeholder="Event name" autoFocus />
          </div>
          <div className="form-group">
            <label className="form-label">Description</label>
            <textarea className="form-textarea" value={eDesc} onChange={e => setEDesc(e.target.value)} rows={2} />
          </div>
          <div className="form-row">
            <div className="form-group">
              <label className="form-label">Start</label>
              <input className="form-input" type="datetime-local" value={eStart} onChange={e => setEStart(e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">End</label>
              <input className="form-input" type="datetime-local" value={eEnd} onChange={e => setEEnd(e.target.value)} />
            </div>
          </div>
          <div className="form-group">
            <label className="form-label">Link to Focus Area</label>
            <select className="form-select" value={eArea} onChange={e => setEArea(e.target.value)}>
      ...
      {showSync && (
        <Modal title="Google Calendar Sync" onClose={() => setShowSync(false)}>
          <p className="text-secondary text-sm mb-16">
            To sync with Google Calendar, you need a Google Cloud API Client ID
            with the Calendar API enabled. Enter it below and authorize access.
          </p>
          <div className="form-group">
            <label className="form-label">Google API Client ID</label>
            <input
          ...
          <p className="text-secondary text-sm mb-16">
            Add this script to your index.html for Google sign-in:<br />
            <code style={{ fontSize: 11, color: 'var(--primary)' }}>
              {'<script src="https://accounts.google.com/gsi/client" async defer></script>'}
            </code>
          </p>
          <div className="modal-actions">
            <button className="btn btn-secondary" onClick={() => setShowSync(false)}>Cancel</button>
            <button className="btn btn-primary" onClick={syncGoogle}>Connect</button>
          </div>
        </Modal>
      )}
    </div>
